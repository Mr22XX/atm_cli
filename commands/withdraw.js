...
module.exports = async (options) => {
  ...
  await db.execute('INSERT INTO transactions (account_id, type, amount, created_at) VALUES (?, "withdraw", ?, NOW())', [user.id, amount]);

  user.balance = newBalance;
  session.saveSession(user);

  console.log(`✅ Penarikan berhasil. Saldo baru: Rp ${newBalance.toFixed(2)}`);
};
